...
exports.tailorResumeData = tailorResumeData;
...
const ResumeSchema = zod_1.z.object({
    personalInfo: zod_1.z.object({
        ...
        github: zod_1.z.string().optional().nullable(),
        portfolio: zod_1.z.string().optional().nullable(),
    }),
    professionalSummary: zod_1.z.string().min(50, "Summary must be at least 50 characters"),
    skills: zod_1.z.array(zod_1.z.object({
        name: zod_1.z.string(),
        proficiency: zod_1.z.string(),
    ...
    education: zod_1.z.array(zod_1.z.object({
        ...
        graduationDate: zod_1.z.string().optional().nullable(),
        gpa: zod_1.z.string().optional().nullable(),
    })),
    ...
    certifications: zod_1.z.array(zod_1.z.object({
        ...
        credentialId: zod_1.z.string().optional().nullable(),
    })),
    languages: zod_1.z.array(zod_1.z.object({
        language: zod_1.z.string(),
        proficiency: zod_1.z.string(),
    })).optional(),
    atsScore: zod_1.z.number().min(0).max(100).optional(),
    keywordMatches: zod_1.z.array(zod_1.z.string()).optional(),
...
const buildSystemPrompt = (targetRole, industry = 'technology') => {
...
const buildUserPrompt = (student, targetRole) => {
    const skillsList = student.skills?.map((s) => `${s.name} (${s.proficiency})`).join(', ') || 'None listed';
    const projectsList = student.projects?.map((p) => `${p.title}: ${p.description} [Tech: ${p.technologies?.join(', ') || 'N/A'}]${p.url ? ` [URL: ${p.url}]` : ''}`).join('\n') || 'None listed';
    const certsList = student.certifications?.map((c) => `${c.name} from ${c.issuer}${c.date ? ` (${c.date})` : ''}`).join('\n') || 'None listed';
    const expList = student.experience?.map((e) => `${e.title} at ${e.company}: ${e.description} [${e.startDate} - ${e.endDate || 'Present'}]`).join('\n') || 'None listed';
    const eduList = student.education?.map((e) => `${e.degree}${e.field ? ` in ${e.field}` : ''} at ${e.school}${e.graduationDate ? ` (${e.graduationDate})` : ''}`).join('\n') || 'None listed';
...
async function generateResume(params) {
    const { student, targetRole, modelProvider = 'gemini-flash', industry = 'technology', currentResume, regenerateSection, } = params;
    const modelConfig = {
        provider: modelProvider,
        temperature: regenerateSection ? 0.3 : 0.7, // Lower temp for targeted edits
        maxTokens: regenerateSection ? 2048 : 4096,
    };
    const model = getModel(modelConfig).withStructuredOutput(ResumeSchema);
    let promptContent;
...
async function tailorResumeData(params) {
...
function calculateATSScore(resume, targetRole) {
    ...
    const hasMetrics = resume.experience.some(exp => /\d+%|\$\d+|\d+ [a-zA-Z]+/.test(exp.description));
    if (hasMetrics)
        score += 10;
...
function generateSuggestions(resume, atsScore) {
    const suggestions = [];
    if (atsScore < 70) {
...
function createFallbackResume(student, targetRole) {
    return {
        personalInfo: {
            name: student.name,
            email: student.email,
